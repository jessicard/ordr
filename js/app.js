...
// AUTO GENERATED
// Has a route, Ember creates
// App.TablesController = Ember.ArrayController.extend();
// App.TableController = Ember.ObjectController.extend();
// Asked for by name in template, Ember creates
...
Ember.Handlebars.registerBoundHelper('money', function(value) {
  ...
  if (cents === 0) {
    ...
    // bug if not cleanly devided
    item = parseInt(dollars, 10) + '.' + cents;
  }
  return item;
});
...
App.Tab = DS.Model.extend({
  ...
  totalCents: function() {
    var allCents = this.get('tabItems').getEach('cents');
    return allCents.reduce(function(accum, item){
      return accum + item;
    }, 0);
